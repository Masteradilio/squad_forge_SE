...
class LocalWorkDelegationContract:
    ...
    def evaluate_delegation(
                ...
                if os.path.isfile(file_path):
                    try:
                        ...
                        if sz > self.max_file_size:
                            return False, f"File {file_rel} size ({sz} bytes) exceeds local model limit ({self.max_file_size} chars)."
        ...
        if task.risk_level.lower() in ("high", "critical"):
            return False, f"Task risk level is {task.risk_level.upper()}, which requires Chief Engineer review/implementation."

        # 4. Check if task description indicates architect scope
        text = f"{task.title} {task.description}".lower()
        if any(term in text for term in ("architecture", "breaking change", "cross-module", "public api")):
