  ...
  return (
    <div style={{ display: 'flex', gap: '16px', overflowX: 'auto', paddingBottom: '16px', minHeight: '600px' }}>
      {columns.map(col => {
        ...
        return (
          <div key={col.id} style={{ flex: '0 0 300px', backgroundColor: '#f3f4f6', padding: '16px', borderRadius: '8px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
            <h3 style={{ margin: '0 0 12px 0', fontSize: '1.1rem', color: '#374151', display: 'flex', justifyContent: 'space-between' }}>
              {col.title} <span style={{ backgroundColor: '#e5e7eb', padding: '2px 8px', borderRadius: '12px', fontSize: '0.8rem' }}>{colTasks.length}</span>
            </h3>
            {colTasks.map(task => {
              const contract = (task as any).metadata?.task_contract;
              ...
              return (
                ...
                >
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    ...
                    {isChief ? (
                      <span title="API Model (Chief Engineer)" style={{ fontSize: '1.2rem' }}>☁️ API</span>
                    ) : (
                      <span title="Local Model" style={{ fontSize: '1.2rem' }}>💻 Local</span>
                    )}
                  </div>
                </div>
              );
