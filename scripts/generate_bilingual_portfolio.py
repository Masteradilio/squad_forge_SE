        ...
        <div class="p-4 rounded-xl bg-slate-950/90 border border-slate-800 h-72 overflow-y-auto space-y-3 font-sans text-xs" id="chat-box">
          <div class="flex gap-2.5">
            ...
            <div class="p-3 rounded-xl rounded-tl-none bg-slate-800 text-slate-200 leading-relaxed max-w-[85%]" id="bot-welcome-msg">
              Olá! Sou o assistente virtual de <strong>Adilio Farias</strong>. Estou conectado aos currículos completos e aos READMEs técnicos dos 7 repositórios públicos. Pergunte o que quiser sobre minhas experiências, educação e projetos!
            </div>
          </div>
        </div>
...
def get_js() -> str:
    ...
    async function processQuestion(question) {
      ...
      try {
        // Live fetch to serverless Cloudflare proxy with 45s timeout for LLM generation
      ...
      if (!answer) {
        ...
        if (qLower.includes('pix') || qLower.includes('fraude') || qLower.includes('fraud') || qLower.includes('sentinel') || qLower.includes('brb')) {
          ...
          answer = kb.credito;
        } else if (qLower.includes('projeto') || qLower.includes('project') || qLower.includes('7') || qLower.includes('github') || qLower.includes('repo') || qLower.includes('squad') || qLower.includes('time_series') || qLower.includes('datasus') || qLower.includes('biblioteca') || qLower.includes('linguag')) {
          answer = kb.projetos;
