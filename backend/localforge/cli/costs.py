...
def list_pricing_sources_cmd():
    ...
    async def run():
        async with UnitOfWork() as uow:
            ...
            if not sources:
                console.print(
                    "[yellow]No pricing sources found in the database. Please bootstrap first.[/yellow]"
                )
                return
            ...
            for s in sources:
                table_sources.add_row(
                    str(s.id), s.provider, s.url, s.retrieved_at.strftime("%Y-%m-%d %H:%M:%S")
                )
...
def add_pricing_source_cmd(
    provider: str = typer.Option(
        ..., "--provider", help="Name of the pricing provider (e.g. OpenAI)."
    ),
    ...
    async def run():
        async with UnitOfWork() as uow:
            ...
            console.print(
                f"[bold green]Successfully added pricing source #{source.id} for {source.provider}.[/bold green]"
            )
...
def update_price_cmd(
    ...
    input_price: float = typer.Option(
        ..., "--input", help="Price per million input tokens in USD."
    ),
    output_price: float = typer.Option(
        ..., "--output", help="Price per million output tokens in USD."
    ),
    cached_input_price: float = typer.Option(
        0.0, "--cached", help="Price per million cached input tokens in USD."
    ),
):
    ...
    async def run():
        async with UnitOfWork() as uow:
            ...
            console.print(
                "[bold green]Successfully updated model pricing snapshot "
                f"#{snapshot.id} for {snapshot.model_name}.[/bold green]"
            )
        ...
        async with UnitOfWork() as uow:
            ...
            if active_run_id is None:
                ...
                if row:
                    ...
                    console.print(
                        "[yellow]No runs found in the database. Run scheduler first.[/yellow]"
                    )
                    return
            ...
            table.add_row("Paid API Calls", str(metrics["actual_calls"]), "-", "-", "-")

            table.add_row("Local Calls Saved", str(metrics["local_calls_avoided"]), "-", "-", "-")
...
def cost_simulate_cmd(
    ...
    async def run():
        async with UnitOfWork() as uow:
            ...
            console.print(
                "*Note: Simulated values represent hypothetical 100% API execution based on pricing snapshots.*"
            )
