    ...
    <section id="assistente" class="py-16 sm:py-24 px-4 sm:px-6 lg:px-8 max-w-4xl mx-auto border-t border-slate-800/80">
      <div class="glass-card rounded-2xl p-6 sm:p-8 border border-cyan-500/40 shadow-2xl glow-cyan space-y-6">
        ...
        <form id="chat-form" class="flex gap-2">
          <input type="text" id="chat-input" placeholder="Pergunte sobre experiências, formação, projetos ou contratação..." class="flex-1 px-4 py-2.5 rounded-xl bg-slate-900 border border-slate-700 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-cyan-500">
...
def get_js() -> str:
    ...
    function appendMessage(sender, text, isHtml = false) {
      ...
      const bubble = `<div class="p-3.5 rounded-xl ${isUser ? 'rounded-tr-none bg-cyan-600 text-white shadow-md' : 'rounded-tl-none bg-slate-800 text-slate-200 border border-slate-700/80 shadow-md'} leading-relaxed max-w-[88%] font-sans text-xs">${isHtml ? text : escapeHtml(text)}</div>`;
    ...
    function formatMarkdown(text) {
      ...
      // Temporarily stash fenced code blocks so internal markdown is protected
      const codeBlocks = [];
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        if (/^###\s+(.*)/.test(line)) {
          formattedLines.push(line.replace(/^###\s+(.*)/, '<h4 class="font-bold text-white text-xs mt-3 mb-1.5 flex items-center gap-1.5"><span class="w-1.5 h-1.5 rounded-full bg-cyan-400"></span><span>$1</span></h4>'));
          continue;
        }
        if (/^##\s+(.*)/.test(line)) {
          formattedLines.push(line.replace(/^##\s+(.*)/, '<h3 class="font-bold text-white text-sm mt-3.5 mb-1.5 border-b border-slate-700/60 pb-1">$1</h3>'));
          continue;
        }
        if (/^#\s+(.*)/.test(line)) {
          formattedLines.push(line.replace(/^#\s+(.*)/, '<h2 class="font-extrabold text-white text-sm mt-4 mb-2">$1</h2>'));
          continue;
        }

        // Unordered List: - item, * item, • item
        if (/^[\-\*\•]\s+(.*)/.test(line)) {
          formattedLines.push(line.replace(/^[\-\*\•]\s+(.*)/, '<div class="ml-2 my-1 flex items-start gap-2"><span class="text-cyan-400 mt-0.5 text-xs leading-none">•</span><span class="flex-1 leading-relaxed">$1</span></div>'));
          continue;
        }

        // Ordered List: 1. item, 2. item
        if (/^(\d+)\.\s+(.*)/.test(line)) {
          formattedLines.push(line.replace(/^(\d+)\.\s+(.*)/, '<div class="ml-2 my-1 flex items-start gap-2"><span class="text-cyan-400 font-mono font-bold text-[11px] leading-none">$1.</span><span class="flex-1 leading-relaxed">$2</span></div>'));
          continue;
        }
