...
def test_role_pipeline_scaffolds_initial_calculator_when_model_returns_no_files(tmp_path):
    manager = make_db_manager(tmp_path)
    try:
        ids = seed_pipeline_state(
            manager,
            tmp_path,
            title="Initialize calculator app structure",
            description="Create a runnable calculator app structure",
            metadata={"runtime_actions": json.dumps({"actions": []})},
        )
        ...
        assert data["status"] == TaskStatus.PR_READY.value
        assert str(data["pr"]).endswith("pr.md")
        assert "calculator/core.py" in data["changed_files"]
        assert "tests/test_scaffold.py" in data["changed_files"]
    finally:
        close_manager(manager)


def test_role_pipeline_scaffolds_initial_calculator_after_broken_model_tests(tmp_path):
    manager = make_db_manager(tmp_path)
    try:
        ids = seed_pipeline_state(
            manager,
...
def test_role_pipeline_preserves_calculator_base_exports_for_feature_tasks(tmp_path):
    manager = make_db_manager(tmp_path)
    try:
        calculator_dir = tmp_path / "calculator"
        calculator_dir.mkdir()
        (calculator_dir / "core.py").write_text(
            "class RPNStack:\n"
            "    pass\n\n"
        ...
        tests_dir.mkdir()
        (tests_dir / "test_scaffold.py").write_text(
            "from calculator import Calculator, RPNStack, add\n\n\n"
            "def test_base_exports_remain_available():\n"
            "    assert Calculator().render()\n"
            "    assert RPNStack is not None\n"
            "    assert add(2, 3) == 5\n",
            encoding="utf-8",
        )
        ids = seed_pipeline_state(
            manager,
            tmp_path,
            title="Build Platinum-style outer casing",
            description="Add calculator visual casing while preserving base calculator API",
            metadata={
                "runtime_actions": json.dumps(
                    {
                        "actions": [
        ...
        async def exercise() -> dict[str, object]:
            async with UnitOfWork(manager) as uow:
                ...
                task = await uow.tasks.get_task(ids["task_id"])
                assert task is not None
                content = (tmp_path / "calculator" / "__init__.py").read_text(
                    encoding="utf-8"
                )
                return {
                    "status": task.status.value,
        ...
        assert data["status"] == TaskStatus.PR_READY.value
        assert "calculator/__init__.py" in data["changed_files"]
        assert "class PlatinumCasing" in str(data["content"])
        assert "class Calculator" in str(data["content"])
    finally:
        close_manager(manager)
...
def test_role_pipeline_preserves_hp12c_common_modules_for_feature_tasks(tmp_path):
    manager = make_db_manager(tmp_path)
    try:
        calculator_dir = tmp_path / "calculator"
        calculator_dir.mkdir()
        ...
        tests_dir = tmp_path / "tests"
        tests_dir.mkdir()
        (tests_dir / "test_rpn_stack.py").write_text(
            "from rpn_stack import FourLevelRPNStack\n\n\n"
            "def test_rpn_stack_imports_after_feature_generation():\n"
            "    stack = FourLevelRPNStack()\n"
            "    stack.push(2)\n"
            "    stack.push(3)\n"
        ...
        ids = seed_pipeline_state(
            manager,
            tmp_path,
            title="Implement four-level RPN stack",
            description="Implement HP 12C calculator RPN behavior",
            metadata={
                "runtime_actions": json.dumps(
                    {
                        "actions": [
        ...
        async def exercise() -> dict[str, object]:
            async with UnitOfWork(manager) as uow:
                ...
                task = await uow.tasks.get_task(ids["task_id"])
                assert task is not None
                content = (tmp_path / "rpn_stack.py").read_text(encoding="utf-8")
                return {
                    "status": task.status.value,
                    "changed_files": task.metadata.get("changed_files", []),
                    "content": content,
        ...
        assert data["status"] == TaskStatus.PR_READY.value
        assert "rpn_stack.py" in data["changed_files"]
        assert "class FourLevelRPNStack" in str(data["content"])
        assert "def broken" not in str(data["content"])
    finally:
        close_manager(manager)


def test_role_pipeline_adds_hp12c_import_alias_modules(tmp_path):
    manager = make_db_manager(tmp_path)
    try:
        calculator_dir = tmp_path / "calculator"
        calculator_dir.mkdir()
        ...
        numeric_pkg = tmp_path / "numeric_entry"
        numeric_pkg.mkdir()
        (numeric_pkg / "__init__.py").write_text(
            "from .numeric_entry import NumericEntry\n",
            encoding="utf-8",
        )
        tvm_pkg = tmp_path / "tvm"
        tvm_pkg.mkdir()
        (tvm_pkg / "__init__.py").write_text("class TVM:\n    pass\n", encoding="utf-8")
        tests_dir = tmp_path / "tests"
        tests_dir.mkdir()
        (tests_dir / "test_aliases.py").write_text(
            "from numeric_entry import NumericEntry\n"
            "from tvm import tvm_solver\n\n\n"
            "def test_aliases_import():\n"
            "    assert NumericEntry().value() == 0\n"
            "    assert tvm_solver.solve(n=1).n == 1\n",
            encoding="utf-8",
        )
        (tests_dir / "test_more_aliases.py").write_text(
            "from components.lcddisplay import LCDDisplay\n"
            "from localforge.display import ModeIndicators\n"
            "from localforge.shift_state import ShiftState\n"
            "from src.casing import PlatinumCasing\n"
            "from statistics.statistics import StatisticsRegister\n\n\n"
            "def test_more_aliases_import():\n"
            "    assert 'Platinum' in PlatinumCasing().describe()\n"
            "    assert LCDDisplay('12.00').render()\n"
            "    assert ModeIndicators().render() == ''\n"
            "    assert ShiftState().active is None\n"
            "    stats = StatisticsRegister(); stats.add(2); stats.add(4)\n"
            "    assert stats.mean() == 3\n",
            encoding="utf-8",
        )
        ids = seed_pipeline_state(
            manager,
            tmp_path,
            title="Implement numeric entry",
            description="Implement HP 12C calculator numeric input",
            metadata={"runtime_actions": json.dumps({"actions": []})},
        )
        ...
        assert asyncio.run(exercise()) == TaskStatus.PR_READY.value
        assert (numeric_pkg / "numeric_entry.py").exists()
        assert (tvm_pkg / "tvm_solver.py").exists()
        assert (tmp_path / "src" / "casing.py").exists()
        assert (tmp_path / "components" / "lcddisplay.py").exists()
        assert (tmp_path / "localforge" / "shift_state.py").exists()
        assert (tmp_path / "localforge" / "display.py").exists()
    finally:
        close_manager(manager)
